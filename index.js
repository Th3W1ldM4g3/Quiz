...
const easyQuestions= ["33 + 77 = x", "75 * 5 = x", "40 / 8 = x", "100 - (-150) = x", "√(25) = x", "150 + 373 = x", "33 * 7 = x",  "480 / 8 = x", "1.111.111 - 2 = x",  "(945 * 30^5 / √(100^4) + 250^50^5) * 0 = x"];
const hardQuestions = ["12 ^ 2 = x", "√(144/121) = x", "³√(4^6) = x",  "10! / 8! = x", "13 ^ 2 - 11 ^ 2 = x", "³√(4^6 / 2^9) = x", "4^(2/4) / 16^(1/2) = x", "(1950! / 1948!) / (1949! / 1948!) = x", "³√(27! / 26!) = x", "TEMPTEMPTEMPTEMPTEMPTEMPTEMPTEMPTEMPTEMPTEMPTEMPTEMPTEMPTEMPTEMPTEMPTEMPTEMPTEMPTEMPTEMPTEMPTEMP" ];
...
const answers12 = ["X = 16", "X = 64", "X = 8", "X = 32"];
...
function checkAnswer(answer) {
    if(answer.innerHTML == tempCorrectAnswer) {
        ...
        location.reload
        difficultyButtonEasy.style.display = 'inline-block';
        difficultyButtonHard.style.display = 'inline-block';
        formAnswers.style.display = 'none';
        textQuestion.textContent = 'Bem vindo ao quiz de Matemática!'
    }
}
